...
Every event rejects unknown fields. Key sequences and mapping key lists must be non-empty and cannot contain empty tokens. Both collector and analyzer enforce a 64 KiB encoded event-line limit, session IDs are limited to 128 bytes, and retained key tokens and mapping IDs are limited to 256 encoded bytes each. The collector losslessly splits a large key sequence before it reaches that shared line boundary.
