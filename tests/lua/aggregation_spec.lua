...
local retry = { callback = nil, now_ms = 0, scheduled = {}, write_state = "partial" }
...
local retry_collector = collector.new({
  ...
  schedule = function(callback)
    table.insert(retry.scheduled, callback)
  end,
})
...
assert(retry_collector:status().last_error == nil, "callback-path writes must be deferred")
assert(#retry.scheduled == 1)
table.remove(retry.scheduled, 1)()
assert(retry_collector:status().state == "recording")
assert(retry_collector:status().pending_events == 1)
assert(retry_collector:status().last_error == "collector pending write failed")
assert(string.find(vim.inspect(retry_collector:status()), "injected aggregation write failure", 1, true) == nil)
