...
assert(defaults.privacy.raw_keylog == false, "raw key logging must remain unavailable")
assert(defaults.privacy.capture_control_keys == false, "control-key capture must be opt-in")
...
assert(pcall(config.resolve, { storage = { directory = "" } }) == false)
...
local control_key_options = config.resolve({ privacy = { capture_control_keys = true } })
assert(control_key_options.privacy.capture_control_keys == true)
assert(pcall(config.resolve, { privacy = { capture_control_keys = "yes" } }) == false)
assert(pcall(config.resolve, { privacy = { capture_insert_text = true } }) == false)
assert(pcall(config.resolve, { privacy = { capture_command_text = true } }) == false)
assert(pcall(config.resolve, { privacy = { capture_search_text = true } }) == false)
assert(pcall(config.resolve, { privacy = { store_file_paths = true } }) == false)
assert(pcall(config.resolve, { unknown = true }) == false)
assert(pcall(config.resolve, { storage = { retention = { typo = 1 } } }) == false)
local partial = config.resolve({ collection = { max_sequence_keys = 128 } })
assert(partial.collection.max_sequence_keys == 128)
assert(partial.collection.exclude_special_buffers == true)
...
assert(config.is_excluded_buffer({ buftype = "terminal", filetype = "" }, {
  collection = { exclude_special_buffers = false },
}), "special buffers must remain force-excluded even with an unvalidated options table")
